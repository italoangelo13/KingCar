...
$qtdeMsg = $Contato->SelecionarNumSolicitacaoContatosNaoLidos();

if (count($qtdeMsg)) {
    foreach ($qtdeMsg as $row) {
        $numMsg = $row->NUMCONTATOS;
    }
}
...
<div class="row bg-primary text-white">
    <div class="col-lg-8">
        <h5>Solicitações de Contato</h5>
    </div>
    <div class="col-lg-4 text-right">
         <label id="_lblNumCont" for="">0</label> Registro(s) não Lido(s)!
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        setInterval("atualizaNumContatos()", 1000);
    });
    ...
    function atualizaNumContatos() {
        if (sessionStorage.getItem('NumContatos')) {
            var cont = sessionStorage.getItem('NumContatos');
            $("#_lblNumCont").text(cont);
        } else {
            $("#_lblNumCont").text("0");
        }
    }
